...
const handler= async (req, res)=> {
    if(req.method=== "GET") {
        try {
            ...
            const [rows]= await connection.execute("SELECT *, score.score_id AS id FROM score INNER JOIN course ON course.course_id = score.course_id INNER JOIN class ON class.class_id = score.class_id INNER JOIN student ON student.student_id = score.student_id WHERE score.class_id= ? AND score.course_id= ?", [class_id, course_id])
